...
def test_addr_properties():
    ...
    assert False == url.supports_routing
    assert True == email.supports_routing

    assert Address.Type.Email == email.addr_type
    assert Address.Type.Url == url.addr_type
    assert non_ascii == "gonz@host.com"

    adr = parse("Zeka <EV@host.coM>")
    assert str(adr) == 'EV@host.com'
...
def test_address_compare():
    ...
    assert a == a.address
    assert not (a != a.address)

    assert b != a.address
    assert not (b == a.address)


def test_local_url():
    u = UrlAddress('http:///foo/bar')
    assert None == u.hostname


def test_addresslist_basics():
    lst = parse_list("http://foo.com:1000; Biz@Kontsevoy.Com   ")
    assert 2 == len(lst)
    assert "http" == lst[0].scheme
    assert "kontsevoy.com" == lst[1].hostname
    assert "Biz" == lst[1].mailbox
    assert "biz@kontsevoy.com" in lst

    # test case-sensitivity: hostname must be lowercased, but the local-part needs
    # to remain case-sensitive
    assert "Biz@kontsevoy.com" in str(lst)
    ...
    assert len(lst) == 4
    assert "http://foo.com:8080" == lst[0].address
    assert "ev@host.com" == lst[1].address
    assert "alex@yahoo.net" == lst[2].address
    assert '"tom+[a]"@s.com' == lst[3].address
    ...
    # hostnames:
    assert {'host.com', 'foo.com', 'yahoo.net', 's.com'} == lst.hostnames
    assert {'url', 'email'} == lst.addr_types
...
def test_addresslist_with_apostrophe():
    ...
    lst = parse_list(s)
    assert 2 == len(lst)
    assert 'Allan G\'o <allan@example.com>' == lst[0].full_spec()
    assert 'Os Wi <oswi@example.com>' == lst[1].full_spec()
    lst = parse_list("Eugueny ώ Kontsevoy <eugueny@gmail.com>")
    assert '=?utf-8?q?Eugueny_=CF=8E_Kontsevoy?= <eugueny@gmail.com>' == lst.full_spec()
    assert u'Eugueny ώ Kontsevoy' == lst[0].display_name
...
def test_addresslist_non_ascii_list_input():
    ...
    lst = parse_list(al)
    assert 2 == len(lst)
    assert '=?utf-8?q?Aur=C3=A9lien_Berger?= <ab@example.com>' == lst[0].full_spec()
    assert 'Os Wi <oswi@example.com>' == lst[1].full_spec()
...
def test_addresslist_address_obj_list_input():
    ...
    assert 2 == len(lst)
    assert '=?utf-8?q?Aur=C3=A9lien_Berger?= <ab@example.com>' == lst[0].full_spec()
    assert 'https://www.example.com' == lst[1].full_spec()


def test_edge_cases():
    email = EmailAddress('"foo.bar@"@example.com')
    assert '"foo.bar@"@example.com' == email.address


def test_display_name__to_full_spec():
    assert '"foo (\\"bar\\") blah" <foo@bar.com>' == EmailAddress('foo ("bar") blah', 'foo@bar.com').full_spec()
    assert 'foo. bar <foo@bar.com>' == EmailAddress('foo. bar', 'foo@bar.com').full_spec()
    assert '"\\"\\"" <foo@bar.com>' == EmailAddress('""', 'foo@bar.com').full_spec()
    assert '=?utf-8?b?0J/RgNC40LLQtdGCINCc0LXQtNCy0LXQtA==?= <foo@bar.com>' == EmailAddress(u'Привет Медвед', 'foo@bar.com').full_spec()
...
def test_address_properties_req_utf8():
    for i, tc in enumerate([{
        ...
        assert tc['repr'] == repr(addr_list)
        assert tc['str'] == str(addr_list)
        assert tc['unicode'] == addr_list.to_unicode()
        if isinstance(tc['full_spec'], Exception):
            with pytest.raises(type(tc['full_spec'])):
                addr_list.full_spec()
        else:
            assert tc['full_spec'] == addr_list.full_spec()


def test_address_full_spec_smart_quote_display_name():
    assert EmailAddress('foo',         'foo@bar.com').full_spec() == 'foo <foo@bar.com>'
    assert EmailAddress('()<>[]:;@,.', 'foo@bar.com').full_spec() == '"()<>[]:;@,." <foo@bar.com>'
    assert EmailAddress('"',           'foo@bar.com').full_spec() == '"\\"" <foo@bar.com>'
    assert EmailAddress('\\',          'foo@bar.com').full_spec() == '"\\\\" <foo@bar.com>'


def test_address_unicode_smart_quote_display_name():
    assert EmailAddress('foo',         'foo@bar.com').to_unicode() == u'foo <foo@bar.com>'
    assert EmailAddress('()<>[]:;@,.', 'foo@bar.com').to_unicode() == u'"()<>[]:;@,." <foo@bar.com>'
    assert EmailAddress('"',           'foo@bar.com').to_unicode() == u'"\\"" <foo@bar.com>'
    assert EmailAddress('\\',          'foo@bar.com').to_unicode() == u'"\\\\" <foo@bar.com>'
    assert EmailAddress('Федот',       'foo@bar.com').to_unicode() == u'Федот <foo@bar.com>'
    assert EmailAddress('<Федот>',     'foo@bar.com').to_unicode() == u'"<Федот>" <foo@bar.com>'


def test_contains_non_ascii():
    assert EmailAddress(None, 'foo@bar.com').contains_non_ascii() == False
    assert EmailAddress(None, 'foo@экзампл.рус').contains_non_ascii() == True
    assert EmailAddress(None, 'аджай@bar.com').contains_non_ascii() == True
    assert EmailAddress(None, 'аджай@экзампл.рус').contains_non_ascii() == True


def test_contains_domain_literal():
    assert EmailAddress(None, 'foo@bar.com').contains_domain_literal() == False
    assert EmailAddress(None, 'foo@[1.2.3.4]').contains_domain_literal() == True


def test_parse_relaxed():
    assert u'foo <foo@bar.com>' == parse('foo <foo@bar.com>').to_unicode()
    assert u'foo <foo@bar.com>' == parse('foo foo@bar.com').to_unicode()
    assert u'foo <foo@bar.com>' == parse('foo (comment) <foo@bar.com>').to_unicode()
    assert u'"foo (comment)" <foo@bar.com>' == parse('foo (comment) foo@bar.com').to_unicode()
    assert u'"not@valid" <foo@bar.com>' == parse('not@valid <foo@bar.com>').to_unicode()
    assert u'"not@valid" <foo@bar.com>' == parse('not@valid foo@bar.com').to_unicode()
    assert u'Маруся <мария@example.com>' == parse('Маруся мария@example.com').to_unicode()
...
def test_parse_list_relaxed():
    ...
    assert expected == [addr.to_unicode() for addr in parse_list(addr_list)]
