...
def encoded_word_test():
    ...
    assert r[0] == 'utf-8'
    assert r[1] == 'B'
    assert r[2] == 'U2ltcGxlIHRleHQuIEhvdyBhcmUgeW91PyDQmtCw0Log0YLRiyDQv9C+0LY='

    r = t('=?UTF-8?Q?=D1=80=D1=83=D1=81=D1=81=D0=BA=D0=B8=D0=B9?=')
    assert r[0] == 'UTF-8'
    assert r[1] == 'Q'
    assert r[2] == '=D1=80=D1=83=D1=81=D1=81=D0=BA=D0=B8=D0=B9'

    r = t('=?iso-8859-1?q?this=20is=20some=20text?=')
    assert r[0] == 'iso-8859-1'
    assert r[1] == 'q'
    assert r[2] == 'this=20is=20some=20text'
...
def happy_mime_to_unicode_test():
    ...
    assert u'Simple text. How are you? Как ты поживаешь?' == encodedword.mime_to_unicode(v)

    v = ' =?US-ASCII?Q?Foo?= <foo@example.com>'
    assert u'Foo <foo@example.com>' == encodedword.mime_to_unicode(v)

    v = '''=?UTF-8?Q?=D1=80=D1=83=D1=81=D1=81=D0=BA=D0=B8=D0=B9?=\n     =?UTF-8?Q?_=D0=B8?= english112      =?UTF-8?Q?=D1=81=D0=B0=D0=B1=D0=B6?= subject'''
    assert u'русский и english112      сабж subject' == encodedword.mime_to_unicode(v)

    v = '=?iso-8859-1?B?SOlhdnkgTel05WwgVW7uY/hk?=\n\t=?iso-8859-1?Q?=E9?='
    assert u'Héavy Métål Unîcødé' == encodedword.mime_to_unicode(v)
...
def lying_encodings_mime_to_unicode_test():
    ...
    assert u'русский  english112      сабж subject' == encodedword.mime_to_unicode(v)
...
\tfor <alex@mailgun.net>; Fri, 17 Dec 2010 12:50:07 +0000 (UTC)'''
    assert u'from mail-iy0-f179.google.com (mail-iy0-f179.google.com\t[209.85.210.179])\tby mxa.mailgun.org (Postfix) with ESMTP id 2D0D3F01116\tfor <alex@mailgun.net>; Fri, 17 Dec 2010 12:50:07 +0000 (UTC)' == encodedword.mime_to_unicode(v)

    v = '''multipart/mixed; boundary="===============7553021138737466228=="'''
    assert v == encodedword.mime_to_unicode(v)
...
def outlook_encodings_test():
    ...
    assert u"Это сообщение с длинным сабжектом специально чтобы проверить кодировки" == encodedword.mime_to_unicode(v)
...
def various_encodings_test():
    ...
    assert u'"考取分享" <foo@example.com>' == encodedword.mime_to_unicode(v)

    v = """=?UTF-8?B?0JbQtdC60LA=?= <ev@mailgun.net>, =?UTF-8?B?0JrQvtC90YbQtdCy0L7QuQ==?= <eugueny@gmail.com>"""
    assert u"Жека <ev@mailgun.net>, Концевой <eugueny@gmail.com>" == encodedword.mime_to_unicode(v)
    ...
    assert u"Концевой <ev@host.com>, Bob <bob@host.com>, Винс <vince@host.com>" == v

    v = '=?UTF-8?B?0J/RgNC+0LLQtdGA0Y/QtdC8INGA0YPRgdGB0LrQuNC1INGB0LDQsdC2?=\n =?UTF-8?B?0LXQutGC0Ysg0Lgg0Y7QvdC40LrQvtC0IOKYoA==?='
    assert u'Проверяем русские сабжекты и юникод ☠' == encodedword.mime_to_unicode(v)

    v = '=?UTF-8?B?0J/RgNC+0LLQtdGA0Y/QtdC8INGA0YPRgdGB0LrQuNC1INGB0LDQsdC2?=\r\n =?UTF-8?B?0LXQutGC0Ysg0Lgg0Y7QvdC40LrQvtC0IOKYoA==?='
    assert u'Проверяем русские сабжекты и юникод ☠' == encodedword.mime_to_unicode(v)

    v = '=?utf-8?Q?Evaneos-Concepci=C3=B3n.pdf?='
    assert u'Evaneos-Concepción.pdf' == encodedword.mime_to_unicode(v)

    v = u'=?gb2312?Q?Hey_There=D7=B2=D8=B0?='
    assert u'Hey There撞匕' == encodedword.mime_to_unicode(v)

    v = u'=?gb18030?Q?Hey_There=D7=B2=D8=B0?='
    assert u'Hey There撞匕' == encodedword.mime_to_unicode(v)

    v = parse(u'Тест длинного дисплей нейма <test@example.com>')
    assert v.display_name == encodedword.mime_to_unicode(v.ace_display_name)


@patch.object(utils, '_guess_and_convert', Mock(side_effect=errors.EncodingError()))
def test_convert_to_utf8_unknown_encoding():
    assert u"abc\u20acdef" == charsets.convert_to_unicode("windows-874", b"abc\x80def")
    assert u"qwe" == charsets.convert_to_unicode('X-UNKNOWN', u'qwe')
    assert u"qwe" == charsets.convert_to_unicode('ru_RU.KOI8-R', 'qwe')
    assert u"qwe" == charsets.convert_to_unicode('"utf-8"; format="flowed"', 'qwe')


@patch.object(encodedword, 'unfold', Mock(side_effect=Exception))
def test_error_reporting():
    assert "Sasha" == encodedword.mime_to_unicode("Sasha")
